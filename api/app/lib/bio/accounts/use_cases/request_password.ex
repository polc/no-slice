defmodule Bio.Accounts.UseCases.RequestPassword do
  @moduledoc """
  ...
  alias Bio.Accounts.Schemas.PasswordRequest
  ...
  def call(input) do
    ...
    |> Multi.run(:user, fn _, %{changeset: changeset} ->
      case Bio.Accounts.Dataloader.find_user_by_email(input.email) do
        nil -> {:error, add_error(changeset, :email, "Unable to find user.")}
        user -> {:ok, user}
      end
    end)
    |> Multi.run(:validation, fn _, %{changeset: changeset, user: user} ->
      case Bio.Accounts.Dataloader.count_valid_password_requests(user.id) do
        n when n < 3 ->
          {:ok, nil}
